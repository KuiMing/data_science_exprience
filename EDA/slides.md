...
temp
```
<img src=media/country_3.png width="70%">
...
```python [1-2|3-9|10-20|21-23]
countries = {}
netflix['country'] = netflix['country'].fillna('Unknown')
for i in list(netflix['country']):
  temp = i.replace(", ", ",").split(',')
  if len(temp) == 1:
    if temp[0] in list(countries.keys()):
      countries[temp[0]] += 1
    else:
      countries[temp[0]] = 1
  else:
...
```python
text = ", ".join(list(netflix.listed_in.values))
text
```
...
```python
netflix[netflix.cast.str.contains("Anupam Kher") == True]
```
